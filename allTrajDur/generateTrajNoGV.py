...
# Parse a *.dur.expt file to get the duration means in a vector and duration variances in a vector
def getDuration(durationFile):
    ...
    for time in times:
        ...
        for i in range(0,5):
            timeNumber = float(timeNumbers[i])#int(round(float(timeNumbers[i])))
            uDurMean.append(timeNumber)
        for i in range(5,10):
            timeNumber = float(timeNumbers[i])
            uDurVar.append(timeNumber) 

    #pdb.set_trace()
    uDurMean = np.asarray(uDurMean)
    uDurVar = np.asarray(uDurVar)

    uDur = getDurationExpert(uDurMean,uDurVar)

    return uDur

# Use PoE to obtain a better uDur
def getDurationExpert(uDurMean, uDurVar):
    # Create the stacked mean vector
    numberOfPhones = uDurMean.shape[0]/5
    uDurMeanBar = np.zeros([(numberOfPhones*6), 1]) # 6 is from having 5 states per phone and then a summed value at the end
    pdb.set_trace()
    # Fill in the means
    for i in range(0,int(numberOfPhones*5)):
        uDurMeanBar[i] = uDurMean[i]
    # Fill in the mean sums
    for i in range(numberOfPhones*5, numberOfPhones*6):
        uDurMeanBar[i] = np.sum(uDurMean[i-(numberOfPhones*5):i-(numberOfPhones*5)+4,:])

    # Create the sigmas matrix
...
def createTrajectory(meansBar, sigmasBarMatrix, weightMatrix):
    ...
    #pdb.set_trace() 
    return means, sigmasMatrix

if __name__ == "__main__":
    uDur = getDuration('../experts/utt1/dim1/utt1.dur.expt')
    means, sigmas = getHatStats('../experts/utt1/dim1/utt1.cmp.expt')
    meansBar, sigmasBarMatrix = getBarStats(means,sigmas,uDur)
    weightMatrix = createWeightMatrix(uDur)
    means, sigmasMatrix = createTrajectory(meansBar,sigmasBarMatrix,weightMatrix)
    #pdb.set_trace() 
    with open('dimensionMean4.txt','w+') as f:
        np.savetxt(f,means)
    print(means.var())
    with open('dimensionSigma4.txt','w+') as f:
